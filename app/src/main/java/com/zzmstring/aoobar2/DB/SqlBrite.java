 ...
 */
package com.zzmstring.aoobar2.DB;
...
/**
 * A lightweight wrapper around {@link android.database.sqlite.SQLiteOpenHelper} which allows for continuously observing
 * the result of a query.
 * <p>
 * While not strictly required, instances of this class assume that they will be the only ones
 * interacting with the underlying {@link android.database.sqlite.SQLiteOpenHelper} and it is required for automatic
...
public final class SqlBrite implements Closeable {
   ...
   * <p>
   * By default this method will log verbose message to {@linkplain android.util.Log Android's log}. Use a
  ...
  /**
   * Close the underlying {@link android.database.sqlite.SQLiteOpenHelper} and remove cached readable and writeable
  ...
  /**
   * Create an observable which will notify subscribers with a {@linkplain com.zzmstring.aoobar.DB.SqlBrite.Query query} for
   * execution. Subscribers are responsible for <b>always</b> closing {@link android.database.Cursor} instance
   * returned from the {@link com.zzmstring.aoobar.DB.SqlBrite.Query}.
   * <p>
   ...
   * will not trigger a notification if no rows were updated.
   *
   * @see android.database.sqlite.SQLiteDatabase#updateWithOnConflict(String, android.content.ContentValues, String, String[], int)
   */
  public int update(@NonNull String table, @NonNull ContentValues values,
   ...
   * @return true if the transaction was yielded
   *
   * @see android.database.sqlite.SQLiteDatabase#yieldIfContendedSafely(long)
   */
  ...
  private final class Transaction implements SQLiteTransactionListener {
    ...
    final Set<String> triggers = new LinkedHashSet<String>();
